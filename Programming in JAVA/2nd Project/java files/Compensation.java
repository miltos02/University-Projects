    ...
    public static Compensation parse(BufferedReader buff, int i, ExpenseTypeList expenseTypes, Employee empl, double val){
        ...
        try {
            ...
            while (!(line.trim().equals("}"))) {
                ...
                if (lineTokens.countTokens() >= 1) { //checking for if the line is empty
                    ...
                    if (token.toUpperCase().equals("EXPENSE_TYPE")) {
                        if (lineTokens.countTokens() >= 1) {
                            ...
                            if (token.equals("1") || token.equals("2")) {
								//if EXPENSE_CODE is already found, we check the of EXPENSE_TYPE matches with it
...
]								//checking the position of the expense type in the list
                                position = expenseTypes.findExpTypeByCode(expCode);

                                if (position != -1) {
								    //if EXPENSE_TYPE is already found, we check the of EXPENSE_CODE matches with it
